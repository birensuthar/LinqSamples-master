...
//Sum() totals a specific field/expression. Thus you will likely need to use a delegate to indicate the collection instant attribute to be used.
//Average() averages a specific field/expression, thus you will likely need to use a delegate to indicate the collection instant attribute to be used.
from x in Albums
orderby x.Title
...
select new
{
	...
	AvgTrackLengthInSecondsB = x.Tracks.Average(y => y.Milliseconds/1000)
}